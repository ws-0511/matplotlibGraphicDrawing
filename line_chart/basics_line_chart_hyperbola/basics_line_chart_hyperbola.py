...
y_temperature_one = np.random.randint(30, 38, 8)
y_temperature_twe = np.random.randint(20, 28, 8)

# 绘图,通过label指定图例内容,线条颜色,线条风格
plt.plot(x_time, y_temperature_one, label="第一天", color="orange", linestyle=":")
plt.plot(x_time, y_temperature_twe, label="第二天", color="cyan", linestyle="--")
...
# loc="upper left"调整图例的位置,默认是右上角奥
plt.legend(prop=font, loc="upper left")

# 2:保存(保存应在绘图之前进行)
plt.savefig("basics_line_chart_hyperbola.png")
